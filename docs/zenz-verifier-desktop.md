...
1. The Cargo `candle` feature decides whether Candle and the verifier backend
   are included in the binary. It is on by default; `--no-default-features`
   produces a verifier-free diagnostic build.
2. `CAPTION_BRIDGE_ZENZ_VERIFIER` decides whether an included backend is used
   at runtime. It is on by default.

Both gates are on in the shipped desktop build. A feature-off build cannot load
the backend, and `CAPTION_BRIDGE_ZENZ_VERIFIER=off` is the immediate rollback to
dictionary-only conversion if a device or model has a problem.

The runtime switch is read once at process start:

```sh
CAPTION_BRIDGE_ZENZ_VERIFIER=on   # on/true/1
CAPTION_BRIDGE_ZENZ_VERIFIER=off  # off/false/0 (emergency dictionary-only rollback)
```
...
To disable the default verifier for a launch, set `CAPTION_BRIDGE_ZENZ_VERIFIER=off`
before starting the desktop app. When the `candle` feature is absent (for
example, a `--no-default-features` diagnostic build), an enabled switch keeps
the verifier slot empty. AzooKey still returns its dictionary result and the
shared conversion API reports `CapabilityUnavailable`; it never reports an
...
the dictionary fallback. In particular, an undownloaded
`zenz-v3.2-small-gguf` produces `model_not_found` and does not prevent capture
from starting. Loading is never deferred until a caption is being rendered.
...
The normal quality gate now compiles and lints Candle (and its `onig` C
dependency) because `candle` is a default desktop feature. The verifier crate's
standalone Candle test/lint remains available through
