...
#[test]
fn japanese_morph_reading_uses_surface_kana_and_preserves_unknown_tokens() {
    let source = "音声を行った未知語";
    let tokens = vec![
        JapaneseMorphToken {
            surface: "音声".to_string(),
            char_range: 0..2,
            feature: "名詞,普通名詞,一般,*,*,*,オンセイ,音声,音声,オンセー,音声,オンセー,漢,*,*,*,*,*,*,*,オンセイ".to_string(),
        },
        JapaneseMorphToken {
            surface: "を".to_string(),
            char_range: 2..3,
            feature: "助詞,格助詞,*,*,*,*,ヲ,を,を,オ,を,オ,和,*,*,*,*,*,*,*,ヲ".to_string(),
        },
        JapaneseMorphToken {
            surface: "行った".to_string(),
            char_range: 3..6,
            feature: "動詞,一般,*,*,五段-カ行,連用形-促音便,イク,行く,行った,イッ,行く,イク,和,*,*,*,*,*,*,*,イッタ".to_string(),
        },
        JapaneseMorphToken {
            surface: "未知語".to_string(),
            char_range: 6..9,
            feature: "名詞,普通名詞,一般,*,*,*".to_string(),
        },
    ];

    assert_eq!(
        hiragana_text_from_morph_tokens(source, &tokens),
        "おんせいをいった未知語",
        "surface kana must preserve conjugation while dictionary gaps remain lossless"
    );
}
