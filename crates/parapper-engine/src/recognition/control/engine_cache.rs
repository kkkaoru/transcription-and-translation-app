...
use crate::{
    ...
    model::{
        NamoTurnDetectorModel, asr_model_dir_for, language_id_model_dir,
...
impl AsrEngineCache {
    pub(crate) fn preload_required(
        ...
        for model in config.required_asr_models() {
            if let Err(err) = self.ensure(models_root, config, model) {
    ...
    fn ensure(
        ...
        let model_dir = asr_model_dir_for(models_root, config, model);
...
impl NamoTurnDetectorCache {
    pub(crate) fn preload_required(
        ...
        for model in namo_turn_detector_models_for_config(config) {
            if let Err(err) = self.ensure(models_root, model) {
    ...
    fn ensure(&mut self, models_root: &Path, model: NamoTurnDetectorModel) -> Result<()> {
        if self.engines.contains_key(&model) {
            return Ok(());
        }
        let model_dir = namo_turn_detector_model_dir_from_root(models_root, model);
...
pub(crate) fn build_language_id_engine(
    ...
    let model_dir = language_id_model_dir(models_root);
