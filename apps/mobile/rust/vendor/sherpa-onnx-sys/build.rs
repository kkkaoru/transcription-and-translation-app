...
fn emit_ios_link_directives() -> Result<(), DynError> {
    let target = env::var("TARGET")?;
    let slice = if target.contains("-sim") {
        "ios-arm64_x86_64-simulator"
    } else {
        "ios-arm64"
    };
    let vendor_root = PathBuf::from(env::var("CARGO_MANIFEST_DIR")?)
        .parent()
        .ok_or("sherpa-onnx-sys vendor parent is unavailable")?
        .join("sherpa-onnx-ios");
    let sherpa_dir = vendor_root.join("sherpa-onnx.xcframework").join(slice);
    let onnx_runtime_dir = vendor_root
        .join("onnxruntime.xcframework")
        .join(slice)
        .join("onnxruntime.framework");
    let archives = [
        sherpa_dir.join("libsherpa-onnx.a"),
        onnx_runtime_dir.join("libonnxruntime.a"),
    ];
    for archive in &archives {
        if !archive.is_file() {
            return Err(format!("iOS sherpa-onnx archive is missing: {}", archive.display()).into());
        }
    }
    println!("cargo:rustc-link-search=native={}", sherpa_dir.display());
    println!("cargo:rustc-link-search=native={}", onnx_runtime_dir.display());
