...
describe("caption freshness window", () => {
  ...
  it("keeps hook-restamped clocks visible at t=5001 and t=9400, then blanks at t=9600", () => {
    const text = "食べて";
    const later = (now: number) =>
      applyCaptionFreshnessWindow({
        caption: caption({ sourceText: text, isFinal: false, translationText: "eating" }),
        now,
        graphemePaintedAt: paintedAt(text, 4_500),
        lastGrowthAt: 4_500,
        previousSourceText: text,
      });
  ...
  it("keeps a restamped finalized caption visible at t=5001", () => {
    const text = "うん";
    expect(
      applyCaptionFreshnessWindow({
