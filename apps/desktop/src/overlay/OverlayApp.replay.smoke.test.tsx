...
describe("OverlayApp caption replay", () => {
  ...
  it("does not first-paint a truncated same-id ASR tail over a longer greeting in history", async () => {
    history.pushState({}, "", "/?native=1");
    mocks.getLatestCaption.mockResolvedValue(null);
    mocks.getPipelineStageHistory.mockResolvedValue([
      {
    ...
    try {
      ...
      const painted = nativeRendererRoot(container)?.getAttribute("data-source-text") ?? "";
      expect(painted).toContain("こんにちは");
      expect(painted).not.toBe("きこえますか");
    } finally {
      await act(async () => {
        root.unmount();
        await Promise.resolve();
  ...
  it("does not let a later live same-id ASR tail overwrite a longer greeting", async () => {
    ...
    try {
      ...
      expect(nativeRendererRoot(container)?.getAttribute("data-source-text")).toContain(
        "こんにちは",
      );
  ...
  it("keeps lead plus tail when live ASR emits them as two same-id pieces", async () => {
    const matrix = buildCaptionAbMatrix();
    const rows = [
      matrix.find(
        (row) =>
    ...
    try {
      for (const [index, row] of rows.entries()) {
        ...
        const utteranceId = `parapper:s:1:${50 + index}`;
        await act(async () => {
          pipelineListener?.({
            stage: "asr",
            utteranceId,
  ...
  it("does not repaint previous-session ASR history after idle resume", async () => {
    history.pushState({}, "", "/?native=1");
    mocks.getLatestCaption.mockResolvedValue(null);
  ...
  it("ignores untagged delayed live ASR from the idle session after resume", async () => {
    history.pushState({}, "", "/?native=1");
    mocks.getLatestCaption.mockResolvedValue(null);
  ...
  it("ignores untagged delayed live ASR with a non-parapper id after idle resume", async () => {
    history.pushState({}, "", "/?native=1");
    mocks.getLatestCaption.mockResolvedValue(null);
  ...
  it("ignores untagged delayed ASR history from the idle session after resume", async () => {
    history.pushState({}, "", "/?native=1");
    mocks.getLatestCaption.mockResolvedValue(null);
