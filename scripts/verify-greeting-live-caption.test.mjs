...
describe("greeting live-caption regression harness", () => {
  it("keeps a check-in-able fixture table for こんにちは / きこえますか", () => {
    ...
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "ohayou-hearing-prolonged-ae")?.expectedOverlay,
      "おはようーきこえますか",
    );
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "ohayou-gozaimasu-hearing-prolonged-ae")
        ?.expectedOverlay,
      "おはようございますーきこえますか",
    );
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "concat-hearing-prolonged-kanji")?.expectedOverlay,
      "こんにちはー聞こえますか",
    );
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "sayonara-hearing-prolonged-kanji")
        ?.expectedOverlay,
      "さようならー聞こえますか",
    );
    assert.equal(
    ...
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "ohayou-hearing-prolonged-kanji")?.expectedOverlay,
      "おはようー聞こえますか",
    );
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "ohayou-gozaimasu-hearing-prolonged-kanji")
        ?.expectedOverlay,
      "おはようございますー聞こえますか",
    );
    assert.equal(
      fixtures.sanitize.find((row) => row.id === "concat-hearing-prolonged-oe-kanji")
        ?.expectedOverlay,
      "こんにちはー聞こえますか",
    );
    assert.equal(
